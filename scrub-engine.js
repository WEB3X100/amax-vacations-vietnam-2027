...
function mountScrollWorld(container, config) {
  ...
  const nav = el('nav', 'sw-nav'); nav.setAttribute('aria-label', 'Scenes');
  ...
  if (config.cta && config.cta.label) {
    ...
    c.setAttribute('aria-label', config.cta.label);
    topbar.appendChild(c);
  }

  const stage = el('div', 'sw-stage');
  const copylayer = el('div', 'sw-copylayer');
  const route = el('div', 'sw-route'); route.setAttribute('role', 'navigation'); route.setAttribute('aria-label', 'Scene progress');
  ...
  SEGMENTS.forEach(s => {
    ...
    img.alt = SECTIONS[s.si] ? ((SECTIONS[s.si].title || SECTIONS[s.si].label || '') + '') : '';
    img.decoding = 'async'; img.loading = s.si === 0 ? 'eager' : 'lazy';
    if (s.si === 0) img.setAttribute('fetchpriority', 'high');
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      else if (i === N - 1) cop = before ? 0 : smooth(Math.min(pr / 0.18, 1)); // banner on with the view; holds CTA
